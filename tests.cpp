...
void C_move_time() {
    ...
    for (size_t i = 0; i < 5000; ++i) {
        map1.insert({i, i*i});  //实际上是linked_list
        map2.insert({i, i*i});
        answer.insert({i, i*i});
    }
    //map1.debug();
    ...
    ns copy_ctor, move_ctor, copy_assign, move_assign;
        ...
        //copy_constructed = map1;
        //std::cout<<"sssss";
        auto end1 = std::chrono::steady_clock::now();

        copy_ctor = std::chrono::duration_cast<std::chrono::nanoseconds>(end1 - start1);

        //copy_constructed.debug();
        //std::cout<<"copy count:"<<copy_ctor.count()<<std::endl;
        ...
        move_ctor = std::chrono::duration_cast<ns>(end2 - start2);
