...
public:
    ...
    */
    ...
    //using category = typename std::iterator_traits<std::vector<value_type>>::iterator_category;
    /*
    * Default constructor
    ...
    */
    ...
    HashMap(const HashMap<K,M,H>& cpy) noexcept;

    //copy assignment
    HashMap<K,M,H>& operator= (const HashMap<K,M,H>& rhs);

    //move constructor + assignment
    HashMap(HashMap<K,M,H>&& other) noexcept; // no const + &&
    HashMap<K,M,H>& operator=(HashMap<K,M,H>&& rhs) noexcept; //应该不加explicit

    HashMap(std::initializer_list<value_type> hl);

    HashMap(std::initializer_list<typename  std::vector<std::pair<K,M>>::iterator> &&hl);
    //潜在的坑:这里使用std::vector<value_type>::iterator 不行 因为value_type 是 const K,M type
   // HashMap(std::initializer_list<> hl);
    /*
    * Destructor.
    *
